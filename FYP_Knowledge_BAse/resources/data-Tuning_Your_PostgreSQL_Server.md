...
[Jump to navigation](https://wiki.postgresql.org/wiki/Tuning_Your_PostgreSQL_Server#column-one)[Jump to search](https://wiki.postgresql.org/wiki/Tuning_Your_PostgreSQL_Server#searchInput)
_by Greg Smith, Robert Treat, and Christopher Browne_
  

PostgreSQL ships with a basic configuration tuned for wide compatibility rather than performance. Odds are good the default parameters are very undersized for your system. Rather than get dragged into the details of everything you should eventually know (which you can find if you want it at the [GUC Three Hour Tour](http://www.pgcon.org/2008/schedule/events/104.en.html)), here we're going to sprint through a simplified view of the basics, with a look at the most common things people new to PostgreSQL aren't aware of. You should click on the name of the parameter in each section to jump to the relevant documentation in the PostgreSQL manual for more details after reading the quick intro here. There is also additional information available about many of these parameters, as well as a list of parameters you shouldn't adjust, at [Server Configuration Tuning](https://www.packtpub.com/article/server-configuration-tuning-postgresql). 
## Background Information on Configuration Settings
PostgreSQL settings can be manipulated a number of different ways, but generally you will want them changed in your configuration files, either directly or, starting with PostgreSQL 9.4, through [`ALTER SYSTEM`](http://www.postgresql.org/docs/current/static/sql-altersystem.html). The specific options available change from release to release, the definitive list is in the source code at src/backend/utils/misc/guc.c for your version of PostgreSQL (but the pg_settings view works well enough for most purposes). 
...
## [checkpoint_segments checkpoint_completion_target](http://www.postgresql.org/docs/current/static/runtime-config-wal.html#RUNTIME-CONFIG-WAL-CHECKPOINTS)
**Note** : This applies to PostgreSQL 9.4 and below. [PostgreSQL 9.5 introduced min_wal_size and max_wal_size](https://www.postgresql.org/docs/9.5/release-9-5.html) configuration parameters and removed checkpoint_segments. Please review the release notes and the documentation on [min_wal_size](https://www.postgresql.org/docs/current/runtime-config-wal.html#GUC-MIN-WAL-SIZE), [max_wal_size](https://www.postgresql.org/docs/current/runtime-config-wal.html#GUC-MAX-WAL-SIZE) and [WAL configuration](https://www.postgresql.org/docs/current/wal-configuration.html). 
...
There are many things you can log that may or may not be important to you. You should investigate the documentation on all of the options, but here are some tips & tricks to get you started: 
  ...
  * log_min_duration_statement: Not necessary for everyday use, but this can generate [logs of "slow queries"](https://wiki.postgresql.org/wiki/Logging_Difficult_Queries "Logging Difficult Queries") on your system.
...
There are also external tools such [pgbadger](https://pgbadger.darold.net/) that can analyze Postgres logs, see [Monitoring](https://wiki.postgresql.org/wiki/Monitoring "Monitoring") for a comprehensive list. 
...
After every transaction, PostgreSQL forces a commit to disk out to its write-ahead log. This can be done a couple of ways, and on some platforms other options than the shipped wal_sync_method are considerably faster than the conservative default. open_sync is the most common non-default setting switched to, on platforms that support it but default to one of the fsync methods. See [Tuning PostgreSQL WAL Synchronization](http://www.westnet.com/~gsmith/content/postgresql/TuningPGWAL.htm) for a lot of background on this topic. Note that open_sync writing is buggy on some platforms (such as [Linux](http://lwn.net/Articles/350219/)), and you should (as always) do plenty of tests under a heavy write load to make sure that you haven't made your system less stable with this change. [Reliable Writes](https://wiki.postgresql.org/wiki/Reliable_Writes "Reliable Writes") contains more information on this topic. 
...
Retrieved from "[https://wiki.postgresql.org/index.php?title=Tuning_Your_PostgreSQL_Server&oldid=35856](https://wiki.postgresql.org/index.php?title=Tuning_Your_PostgreSQL_Server&oldid=35856)"
[Categories](https://wiki.postgresql.org/wiki/Special:Categories "Special:Categories"): 
  * [Administration](https://wiki.postgresql.org/wiki/Category:Administration "Category:Administration")
  * [Performance](https://wiki.postgresql.org/wiki/Category:Performance "Category:Performance")


## Navigation menu
### Page actions
  * [Page](https://wiki.postgresql.org/wiki/Tuning_Your_PostgreSQL_Server "View the content page \[c\]")
  * [Discussion](https://wiki.postgresql.org/wiki/Talk:Tuning_Your_PostgreSQL_Server "Discussion about the content page \[t\]")
  * [Read](https://wiki.postgresql.org/wiki/Tuning_Your_PostgreSQL_Server)
  * [View source](https://wiki.postgresql.org/index.php?title=Tuning_Your_PostgreSQL_Server&action=edit "This page is protected.
You can view its source \[e\]")
  * [History](https://wiki.postgresql.org/index.php?title=Tuning_Your_PostgreSQL_Server&action=history "Past revisions of this page \[h\]")


### Page actions
  * [Page](https://wiki.postgresql.org/wiki/Tuning_Your_PostgreSQL_Server "Page")
  * [Discussion](https://wiki.postgresql.org/wiki/Talk:Tuning_Your_PostgreSQL_Server "Discussion")
  * [More](https://wiki.postgresql.org/wiki/Tuning_Your_PostgreSQL_Server#p-cactions)
  * [Tools](https://wiki.postgresql.org/wiki/Tuning_Your_PostgreSQL_Server#p-tb "Tools")


### Personal tools
  * [Log in](https://wiki.postgresql.org/index.php?title=Special:UserLogin&returnto=Tuning+Your+PostgreSQL+Server "You must log in to edit any pages on this site. \[o\]")


[](https://wiki.postgresql.org/wiki/Main_Page)
### Navigation
  * [Main Page](https://wiki.postgresql.org/wiki/Main_Page "Visit the main page \[z\]")
  * [Random page](https://wiki.postgresql.org/wiki/Special:Random "Load a random page \[x\]")
  * [Recent changes](https://wiki.postgresql.org/wiki/Special:RecentChanges "A list of recent changes in the wiki \[r\]")
  * [Help](https://www.mediawiki.org/wiki/Special:MyLanguage/Help:Contents "The place to find out")


### Search
### Tools
  * [What links here](https://wiki.postgresql.org/wiki/Special:WhatLinksHere/Tuning_Your_PostgreSQL_Server "A list of all wiki pages that link here \[j\]")
  * [Related changes](https://wiki.postgresql.org/wiki/Special:RecentChangesLinked/Tuning_Your_PostgreSQL_Server "Recent changes in pages linked from this page \[k\]")
  * [Special pages](https://wiki.postgresql.org/wiki/Special:SpecialPages "A list of all special pages \[q\]")
  * [Printable version](javascript:print\(\); "Printable version of this page \[p\]")
  * [Permanent link](https://wiki.postgresql.org/index.php?title=Tuning_Your_PostgreSQL_Server&oldid=35856 "Permanent link to this revision of this page")
  * [Page information](https://wiki.postgresql.org/index.php?title=Tuning_Your_PostgreSQL_Server&action=info "More information about this page")


[](https://wiki.postgresql.org/wiki/Tuning_Your_PostgreSQL_Server#sidebar "Jump to navigation") [](https://wiki.postgresql.org/wiki/Tuning_Your_PostgreSQL_Server#p-personal "user tools") [](https://wiki.postgresql.org/wiki/Tuning_Your_PostgreSQL_Server#globalWrapper "back to top")
[![Powered by MediaWiki](https://wiki.postgresql.org/resources/assets/poweredby_mediawiki_88x31.png)](https://www.mediawiki.org/)
  * This page was last edited on 31 March 2021, at 09:47.
  * [Privacy policy](https://wiki.postgresql.org/wiki/PostgreSQL_wiki:Privacy_policy)
  * [About PostgreSQL wiki](https://wiki.postgresql.org/wiki/PostgreSQL_wiki:About)
  * [Disclaimers](https://wiki.postgresql.org/wiki/PostgreSQL_wiki:General_disclaimer)
